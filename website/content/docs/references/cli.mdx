...
Build a Panda design system package for distribution. Replaces the previous
`ship` + `emit-pkg` two-step.

Produces, under `<outdir>` (default `dist`):

- `panda.buildinfo.json` — pre-extracted classes for the lib's own source
- `panda.lib.json` — manifest (preset path, importMap, parent designSystem)
- `preset.mjs` — the lib's preset compiled from `preset.ts`

…and patches the package's `package.json` `exports` to expose `./preset`,
`./panda.lib.json`, and `./panda.buildinfo.json`.

```bash
pnpm panda lib

# Watch source files and rebuild on change
pnpm panda lib --watch
```

| Flag                  | Description                                                                                 | Related                                                |
| --------------------- | ------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| `--outdir <dir>`      | Output directory for the manifest, buildinfo, and compiled preset (defaults to `dist`)      | -                                                      |
| `--preset <path>`     | Path to the source preset file (defaults to `<outdir>/../preset.ts`)                        | -                                                      |
| `--watch, -w`         | Watch the lib's source and rebuild on change                                                | [`config.watch`](/docs/references/config#watch)        |
| `--silent`            | Whether to suppress all output                                                              | [`config.logLevel`](/docs/references/config#log-level) |
| `--config, -c <path>` | Path to Panda config file                                                                   | [`config`](/docs/references/config.md)                 |
| `--cwd <path>`        | Current working directory                                                                   | [`config.cwd`](/docs/references/config#cwd)            |
